# -*- coding: utf-8 -*-
"""
Created on Sun Aug 20 15:36:06 2023
...
class SelfAttention(nn.Module):
    def __init__(self, embed_size, heads):
        ...
        self.dense = nn.Linear(heads*self.head_dim, embed_size)
        
    def forward(self, values, keys, query, mask):
        N = query.shape[0]
        values_len, keys_len, query_len = values.shape[1], keys.shape[1], query.shape[1]
        
        # Split embedding into heads pices
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, value, key, query, mask):
        ...
        feed = self.feed_forward(out)
        x = self.dropout(self.norm2(feed + out))
        return x
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.embed_size = embed_size
        self.word_embedding = nn.Embedding(try_vocab_size, embed_size)
        self.position_embedding = nn.Embedding(max_length, embed_size)
        ...
        self.dense = nn.Linear(embed_size, try_vocab_size)
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, x, en_out, src_mask, trg_mask):
        N,seq_len = x.shape
        position = torch.arange(0, seq_len).expand(N, seq_len)
        
        x = self.dropout(self.word_embedding(x) + self.position_embedding(position))
        
        for layer in self.layers:
            x = layer(x, en_out, en_out, src_mask, trg_mask)
...
class Transformer(nn.Module):
    ...
    def forward(self, src, trg):
        ...
        encdr = self.encoder(src,src_mask)
        decdr = self.decoder(trg, encdr, src_mask, trg_mask)
        return decdr
